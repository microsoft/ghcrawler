...
class Request {
  constructor(type, url, context = null) {
    ...
    this.policy = TraversalPolicy.default(type);
  }
  ...
  static adopt(object) {
    ...
    if (object.policy) {
      ...
      TraversalPolicy.adopt(object.policy);
    } else {
      TraversalPolicy.default(this.type);
    }
    return object;
  }
  ...
  static _getResolvedPolicy(request) {
    ...
    return TraversalPolicy.getPolicy(policyOrSpec);
  }
  ...
  _resolvePolicy() {
    ...
    if (typeof this.policy === 'string') {
      ...
      const policy = TraversalPolicy.getPolicy(spec);
      if (!policy) {
