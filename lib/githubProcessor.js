...
class GitHubProcessor {
  ...
  collection(request) {
    ...
    if (linkHeader) {
      ...
      // TODO this is a bit reachy.  need a better way to efficiently queue up requests that we know are good.
  ...
  page(page, request) {
    ...
    document.elements.forEach(item => {
      if (elementType) {
        ...
        request.queue(request.relationship, elementType, item.url, { qualifier: elementQualifier }, request.policy);
      } else {
        ...
        const newRequest = new Request(item.type, `${baseUrl}/${item.id}`, null, request.relationship);
        newRequest.payload = { etag: 1, body: item };
        newRequest.policy = request.policy;
        request.queueRequest(newRequest);
      }
    });
    return document;
  }
  ...
  org(request) {
    ...
    // * hooks
    //
    this._addRootReference(request, 'user', 'user', document.url.replace('/orgs/', '/users/'), `urn:user:${document.id}`);
  ...
  repo(request) {
    ...
    if (document.organization) {
      this._addRootReference(request, 'owner', 'user');
      this._addRootContainer(request, 'organization', 'org');
    } else {
      this._addRootContainer(request, 'owner', 'user');
    }
  ...
  commit(request) {
    ...
    this._addRootReference(request, 'author', 'user');
    this._addRootReference(request, 'committer', 'user');
  ...
  commit_comment(request) {
    ...
    this._addRootReference(request, 'user', 'user');
    return document;
  }
  ...
  pull_request(request) {
    ...
    this._addRootReference(request, 'user', 'user');
    this._addRootReference(request, 'merged_by', 'user');
    this._addRootReference(request, 'assignee', 'user');
    if (document.head.repo) {
      this._addRootContainer(request, 'head', 'repo', document.head.repo.url, `urn:repo:${document.head.repo.id}`);
    }
    this._addRootContainer(request, 'base', 'repo', document.base.repo.url, `urn:repo:${document.base.repo.id}`);
  ...
  issue(request) {
    ...
    this._addRootReference(request, 'user', 'user');
    this._addRootContainer(request, 'repo', 'repo', document.repository_url, context.qualifier);
    this._addRootReference(request, 'assignee', 'user');
    this._addRootReference(request, 'closed_by', 'user');
    if (document.comments_url) {
  ...
  deployment(request) {
    ...
    this._addRootReference(request, 'creator', 'user');
    return document;
  }
  ...
  _addEventBasics(request, qualifier = null) {
    ...
    this._addRootReference(request, 'actor', 'user');
    if (repo) {
      this._addRootReference(request, 'repo', 'repo');
    }
    this._addRootReference(request, 'org', 'org');

    return [document, repo, document.payload];
  }

  _addEventResourceReference(request, repo, name, type = name, qualifier = null) {
    return this._addEventResource(request, repo, name, type, qualifier, 'reference');
  }

  _addEventResourceContains(request, repo, name, type = name, qualifier = null) {
    return this._addEventResource(request, repo, name, type, qualifier, 'contains');
  }

  _addEventResource(request, repo, name, type = name, qualifier = null, relationship = 'reference') {
    ...
    const newRequest = new Request(type, payload[name].url, { qualifier: qualifier }, relationship);
    // TODO figure this out in the new policy modle.  queue needs a relationship.  Likely 'contains' here
    request.queueRequests(newRequest);
    return request.document;
  }

  // resources are always a contains relationship
  _addResource(request, name, type, id, url = null, urn = null, qualifier = null) {
    ...
    request.queue('contains', type, url, { qualifier: qualifier });
  }

  // collections are always 'contains' relationships
  _addCollection(request, name, type, url = null, urn = null) {
    ...
    if (this.isRootType(type)) {
      return request.queue('contains', name, url, { elementType: type });
    }
    const newContext = { qualifier: qualifier, elementType: type };
    newContext.qualifier = request.document._metadata.links.self.href;
    request.queue('contains', name, url, newContext);
  }

  _addRootContainer(request, name, type, url = null, urn = null) {
    return this._addRoot(request, 'belongsTo', name, type, url, urn);
  }

  _addRootReference(request, name, type, url = null, urn = null) {
    return this._addRoot(request, 'reference', name, type, url, urn);
  }

  _addRoot(request, relationship, name, type, url = null, urn = null) {
    ...
    url = url || element.url;
    request.linkResource(name, urn);
    request.queue(relationship, type, url);
  }
