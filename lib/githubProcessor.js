...
// commit comment, issue, issue comment, pull request review comment
  ...
  traffic(request) {
    ...
    const document = request.document;
    const context = request.context;
    this._addResource(request, 'referrers', 'referrers', document.id, `${request.url}/traffic/popular/referrers`, null, `urn:repo:${document.id}`);
    this._addResource(request, 'paths', 'paths', document.id, `${request.url}/traffic/popular/paths`, null, `urn:repo:${document.id}`);
    this._addResource(request, 'views', 'views', document.id, `${request.url}/traffic/views`, null, `urn:repo:${document.id}`);
    this._addResource(request, 'clones', 'clones', document.id, `${request.url}/traffic/clones`, null, `urn:repo:${document.id}`);
    return request.document;
  }
  ...
  _trafficChild(request) {
    request.document.id = moment().utc().format('YYYY_MM_DD');
    request.addSelfLink();
    return request.document;
  }
