...
class GitHubProcessor {
  ...
  page(page, request) {
    ...
    document.elements.forEach(item => {
      if (elementType) {
        ...
        request.queue(newRelationship, elementType, item.url, newContext, null, request.policy);
      } else {
        ...
        const newRequest = new Request(item.type, `${baseUrl}/${item.id}`, newContext, request.relationship);
  ...
  _addEventResource(request, repo, name, type = name, qualifier = null, relationship = 'reference') {
    ...
    newRequest.policy = request.policy.getNextPolicy(request, relationship);
    request.queueRequests(newRequest);
    return request.document;
  }
