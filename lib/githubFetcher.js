...
class GitHubFetcher {
        ...
        return this._getFromGitHub(request, options).then(response => {
          ...
          if (status !== 200 && status !== 304) {
            ...
            if (status === 403 && response.headers['retry-after']) {
              const delay = parseInt(response.headers['retry-after'], 10) * 1000;
              return self._handleSecondaryThrottled(request, delay);
            }
            ...
            if (status === 403 && remaining === 0) {
              ...
              return self._handlePrimaryThrottled(request, resetTime);
            }
  ...
  _getFromGitHub(request, options) {
    ...
    return this.requestor.get(url, options).then(response => {
      ...
      request.addMeta({ status: response.statusCode, fetch: time });
      this.limiter.consume(time, key, request.exhaustToken.bind(request));
      return response;
    });
  }
  ...
  _handleDeferred(request, benchTime) {
    ...
    return request.markDefer('Deferred', `Deferring ${delay} for a token`);
  }

  _handleSecondaryThrottled(request, delay) {
    request.addMeta({ requestId: `${request.response.headers['x-github-request-id']}` });
    request.exhaustToken(Date.now() + delay);
    request.addMeta({ secondaryDelay: delay });
    return request.markRequeue('Throttled', 'GitHub secondary throttling kicked in');
  }

  _handlePrimaryThrottled(request, resetTime) {
    request.addMeta({ requestId: `${request.response.headers['x-github-request-id']}` });
    request.exhaustToken(resetTime);
    request.addMeta({ primaryDelay: resetTime - Date.now() });
  ...
  _checkRetryAfter(request, response) {
    const retryAfter = (parseInt(response.headers['retry-after'], 10) || 0) * 1000;
    if (retryAfter > 0) {
      this.logger.info(`Retry-After delay of ${retryAfter}ms for ${request.toString()}`, request.meta);
      request.addMeta({ retryAfterDelay: retryAfter });
      // request.delay(retryAfter);
      request.exhaustToken(Date.now() + retryAfter);
      return true;
    }
    return false;
  }
  ...
  _checkGitHubRateLimit(request, response) {
    this._checkRetryAfter(request, response);
  ...
  _getTypeDetails(type) {
    const result = {
      ...
      reviews: { Accept: 'application/vnd.github.black-cat-preview+json' },
      review: { Accept: 'application/vnd.github.black-cat-preview+json' },
