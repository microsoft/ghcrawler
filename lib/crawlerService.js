...
class CrawlerService {
  constructor(crawler, options) {
    ...
    this.options.crawler.on('change', this._reconfigure.bind(this));
  }
  ...
  updateConfiguration(patches) {
    ...
    Object.getOwnPropertyNames(sorted).forEach(key => {
      ...
      jsonpatch.apply(this.options[key], patchSet);
      Array.prototype.push.apply(promises, this.options[key].notify(patchSet));
    });
  ...
  _reconfigure(patches) {
    ...
    if (patches.some(patch => patch.path === '/count')) {
