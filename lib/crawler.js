...
class Crawler {
    ...
    return etagPromise.then(etag => {
      ...
      return self.requestor.get(request.url, options).then(githubResponse => {
        ...
        if (status === 304) {
          ...
          if (this._shouldFetchExisting(request)) {
  ...
  _storeDocument(request) {
    if (request.shouldSkip() || !this._shouldStore(request)) {
      return Q(request);
    }
  ...
  _shouldFetchExisting(request) {
    return request.policy.shouldFetchExisting(request) || this.processor.shouldFetchExisting(request);
  }

  _shouldStore(request) {
    return this.store && request.document && request.contentOrigin !== 'cacheOfOrigin';
  }
