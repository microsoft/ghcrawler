...
class Crawler {
  ...
  constructor(queues, store, locker, fetcher, processor, options) {
    ...
    this.processors = Array.isArray(processor) ? processor : [processor];
  ...
  _errorHandler(requestBox, error) {
    ...
    if (requestBox[0]) {
      this.logger.error(error, { correlationId: requestBox[0].meta });
      if (requestBox[0].type === '_errorTrap') {
        return requestBox[0];
      }
      return requestBox[0].markRequeue('Processing Error', error.message);
    }
  ...
  _getRequestWork(requestBox, context) {
    ...
    return this.trace(this.queues.pop(), 'pop')
      .then(request => {
        ...
        request.addMeta({ correlationId: this.counter });
        requestBox[0] = request;
  ...
  _processDocument(request) {
    ...
    return this._logStartEnd('processing', request, () => {
      const processors = this.processors.filter(processor => { return processor.getHandler(request); });
      if (!processors) {
        request.markSkip('Skip', `No handler found for request type: ${request.type}`);
        return request;
      }
      const results = processors.map(processor => { return processor.process(request) }).filter(document => { return document; });
      request.document = results.length ? results[0] : null;
      return request;
    });
  }
  ...
  _queueDead(request, reason) {
    ...
    error._correlationId = request.correlationId;
    this.logger.error(error, request.meta);
  ...
  _createQueuable(request) {
    ...
    if (request.payload) {
      queuable.payload = request.payload;
    }
    return queuable;
  }
