...
class Crawler {
  ...
  // .*\.handoff.*
  // handback
  // ontent-.{4}$       azure-content-dede
  // This is currently Microsoft-specific. https://github.com/Microsoft/ghcrawler/issues/109
  _testRegex(request) {
    if (request.url.includes('/OfficeDocs')) {
      return false;
    }
    const patterns = [/.*-pr\..{2}-.{2}.*/, /.*\..{2}-.{2}.*/, /.*-pr\//, /[.|-]handoff/, /handback/,
      /.*ontent-.{4}\//, /\/acom\//, /\/acom-loc\//, /\/azure-content-internal\//];
