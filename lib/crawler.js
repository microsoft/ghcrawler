...
class Crawler {
  ...
  _fetchSingle(request) {
    ...
    return this.store.etag(request.type, request.url).then(etag => {
      const options = etag ? { headers: { 'If-None-Match': request.etag } } : {};
      return this.requestor.get(request.url, options)
        .then(githubResponse => {
          request.response = githubResponse.body;
          request.response._metadata = { etag: githubResponse.headers.etag };
          return request;
        },
        (err) => {
          // if the current doc is the same as what we've already seen then skip
          if (err && err.response && err.response.statusCode === 304) {
            return this._markSkip(request, 'Unchanged');
          }
  ...
  _fetchCollection(request) {
    return this.requestor.getAll(request.url).then(githubResponse => {
