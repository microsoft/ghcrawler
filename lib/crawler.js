...
class Crawler {
  ...
  _filterSeen(request) {
    if (!request || this.seen[request.crawl.url]) {
      return Q.resolve();
    }
  ...
  _convertToDocument(request) {
    ...
    request.response._metadata = {
      type: request.crawl.type,
      url: request.crawl.url,
  ...
  _processDocument(request) {
    ...
    let document = null;
    switch (request.crawl.type) {
      case 'orgs': {
        document = this._processCollection(request.response, 'login', request.crawl.context);
        break;
      }
      case 'repo': {
        document = this._processRepo(request.response, request.crawl.context);
        break;
      }
      case 'login': {
        document = this._processLogin(request.response, request.crawl.context);
        break;
      }
      case 'repos': {
        document = this._processCollection(request.response, 'repo', request.crawl.context);
        break;
      }
      case 'issues': {
        document = this._processCollection(request.response, 'issue', request.crawl.context);
        break;
      }
      case 'issue': {
        document = this._processIssue(request.response, request.crawl.context);
        break;
      }
      case 'issue_comments': {
        document = this._processCollection(request.response, 'issue_comment', request.crawl.context);
        break;
      }
      case 'issue_comment': {
        document = this._processIssueComment(request.response, request.crawl.context);
        break;
      }
      case 'commits': {
        document = this._processCollection(request.response, 'commit', request.crawl.context);
        break;
      }
      case 'commit': {
        document = this._processCommit(request.response, request.crawl.context);
        break;
      }
    }
  ...
  _deleteFromQueue(request) {
    ...
    return this.queue.done(request.message).then(() => { return request; });
  }
  ...
  _markSeen(request) {
    ...
    this.seen[request.crawl.url] = true;
    return Q.resolve(request);
  }
