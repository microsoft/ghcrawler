...
class Crawler {
  ...
  _completeRequest(request, forceRequeue = false) {
    ...
    debug(`_completeRequest(${request.meta.loopName}:${request.toUniqueString()}): enter (force: ${forceRequeue})`);
    const self = this;
  ...
  _requeue(request) {
    ...
    return Q.try(() => {
      ...
      if (++request.attemptCount > 5) {
        return this._storeDead(request, `Exceeded attempt count for ${request.type}@${request.url}`);
      }
  ...
  _filter(request) {
    ...
    if (!request.type || !request.url) {
      ...
      return this._storeDead(request, `Detected malformed request ${request.toString()}`);
    }
  ...
  _storeDead(request, reason, error = null) {
    request._error = error;
    return request.markDead('Deadletter', reason);
  }

  storeDead(request, reason = null) {
  ...
  _preFilter(requests) {
    ...
    return list.filter(request => {
      if (!request.url || !request.type) {
        this._storeDead(request, `Attempt to queue malformed request ${request.toString()}`);
        return false;
      }
      if (this._shouldFilter(request)) {
  ...
  _shouldFilter(request) {
    ...
    if (type === 'repo' || type === 'repos' || type === 'org' || type === 'update_events') {
  ...
  _testRegex(request) {
    //    const patterns = [/.*-pr\..{2}-.{2}.*/, /.*\..{2}-.{2}.*/, /.*-pr.*/, /.*\.handoff.*/];
