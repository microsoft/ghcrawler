...
// commit comment, issue, issue comment, pull request review comment
  ...
  pull_request(request) {
    ...
    if (document._links.statuses) {
      ...
      this._addCollection(request, 'statuses', 'status', document._links.statuses.href, `${context.qualifier}:commits:${sha}:statuses`);
    ...
    if (document._links.issue) {
      ...
      request.linkCollection('comments', `${context.qualifier}:issues:${document.id}:comments`);
  ...
  issue(request) {
    ...
    this._addRelated(request, 'pull_request', 'pull_request', context.qualifier);
    return document;
  }
