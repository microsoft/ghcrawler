...
class Processor {
  ...
  shouldFetchExisting(request) {
    return !!request.context.relation;
  }
  ...
  traffic(request) {
    request.addRootSelfLink();
    request.queueRoot('trafficReferrers', `${request.url}/traffic/popular/referrers`);
    request.queueRoot('trafficPaths', `${request.url}/traffic/popular/paths`);
    request.queueRoot('trafficViews', `${request.url}/traffic/views`);
    request.queueRoot('trafficClones', `${request.url}/traffic/clones`);
    return request.document;
  }

  trafficReferrers(request) {
    return this._trafficChild(request);
  }

  trafficViews(request) {
    return this._trafficChild(request);
  }

  trafficClones(request) {
    return this._trafficChild(request);
  }

  trafficPaths(request) {
    return this._trafficChild(request);
  }
