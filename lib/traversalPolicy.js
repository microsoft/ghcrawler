...
class TraversalPolicy {
  ...
  static always(map) {
    return new TraversalPolicy('origin', 'always', TraversalPolicy._resolveMapSpec(map));
  }
  ...
  getNextPolicy(name) {
    const map = this.map.getNextMap(name);
    if (!map) {
      return null;
    }
    return new TraversalPolicy(this.fetch, this.freshness, map);
  }
